...
void main() {
  group('map', () {
    test('languageCodes should return matching labels', () {
      expect(_kSampleCardAttributeLabels.l10nValueFromLocale(_kGermanLocale.languageCode), 'Sprache');
      expect(_kSampleCardAttributeLabels.l10nValueFromLocale(_kDutchLocale.languageCode), 'Taal');
      expect(_kSampleCardAttributeLabels.l10nValueFromLocale(_kEnglishLocale.languageCode), 'Language');
    });

    test(
        'resolving with a language code that does not exists should default to the english translation if that is available',
        () {
      expect(_kSampleCardAttributeLabels.l10nValueFromLocale(_kNonExistingLocale.languageCode), 'Language');
    });
    ...
    test(
      ...
      expect({'de': 'Sprache', 'nl': 'Taal'}.l10nValueFromLocale(_kNonExistingLocale.languageCode), 'Sprache');
    });

    test('if no translations are available it should default to an empty string', () {
      expect(<String, String>{}.l10nValueFromLocale(_kNonExistingLocale.languageCode), '');
    });
