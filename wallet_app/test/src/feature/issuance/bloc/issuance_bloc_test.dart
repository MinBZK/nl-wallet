...
void main() {
  ...
  blocTest(
    ...
    setUp: () => when(startIssuanceUseCase.invoke(any)).thenAnswer((_) => throw 'error'),
  ...
  blocTest(
    ...
    setUp: () {
      when(startIssuanceUseCase.invoke(any)).thenAnswer(
        (_) async => StartIssuanceReadyToDisclose(
          relyingParty: WalletMockData.organization,
          policy: WalletMockData.policy,
          requestedAttributes: {},
        ),
      );
      when(continueIssuanceUseCase.invoke()).thenAnswer((_) async => ContinueIssuanceResult([WalletMockData.altCard]));
      when(acceptIssuanceUseCase.invoke(any)).thenAnswer((_) async {});
    },
    act: (bloc) {
