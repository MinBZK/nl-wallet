...
The attestation server receives an HTTP `POST` by the `issuance_server` containing a JSON structure of the same form as the `/disclosed_attributes` endpoint of the `verification_server` returns, as documented in the [verifier API documentation](../../get-started/create-a-verifier.md#verifier-api-specifications). More specifically, this is a JSON-serialized `IndexMap<String, DocumentDisclosedAttributes>`. For example:
...
For the rest of the configuration parameters of the `issuance_server`, see the [`issuance_server` example configuration file](../../../wallet_core/wallet_server/issuance_server/issuance_server.example.toml) and the [verifier api specifications](../../get-started/create-a-verifier.md#verifier-api-specifications).
