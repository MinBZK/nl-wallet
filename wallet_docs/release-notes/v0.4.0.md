...
- The Wallet Provider no longer tracks in the database for each user if that
  user received a WIA. The corresponding column in the WP's user table has been
...
- The WTE acronym has been replaced by WIA across the codebase, including in the
...
- The database table `used_wias` (previously known as `used_wtes`) is removed.
...
- When disclosing SD-JWT attestations, the wallet can now disclose an
  ...
  attestations.
- The Wallet Provider now keeps track of the issued WIAs and their revocation
  status.
...
- All Rust services now exposes a MicroProfile compatible `/health` endpoint for
  probing liveness, readiness and started.
...
- The token status list generated by our services now always contain an exp
  ...
  expiration.
- Wallet Provider errors are now measured and exposed via the `/metrics`
  endpoint.
