...
- The wallet now stores the signed wallet configuration (as a JWT) on disk.
  On startup, the stored configuration is loaded and its signature verified,
  preventing a tampered configuration from being used without detection.
  When a newer configuration is fetched from the server, it replaces the
