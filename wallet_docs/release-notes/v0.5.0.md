...
- Verifier certificates must now contain a `san_dns_name` that matches the
  hostname in the OpenID4VP `response_uri`. If these do not match, disclosure
...
- The pid issuer now contains a `client_id` setting in the `digid` section,
  which should equal the DigiD client ID in the wallet configuration.
...
- The Wallet Provider now has an endpoint to revoke a wallet based on a user's
  ...
  revoke their wallet.
- The Wallet Provider now has an endpoint to revoke the solution which will
  effectively revoke all wallets and disallow registering new wallets. This will
  be used by the Wallet Provider's admin interface. The endpoint is disabled by
  default and needs to be enabled via a setting change.
- The Wallet Provider now has an endpoint to list and remove recovery codes from
  the deny list.
...
- When the wallet has been revoked by the Wallet Provider, it will now inform
  the user and reset the wallet to its initial state if the revocation was
  initiated remotely by the user.
- Updated Flutter to 3.41.2
- The wallet now stores the signed wallet configuration (as a JWT) on disk. On
  startup, the stored configuration is loaded and its signature verified,
  preventing a tampered configuration from being used without detection. When a
  newer configuration is fetched from the server, it replaces the previously
  stored one.
- Introduce the 'Present QR' screen, this is the entry point for close proximity
  disclosure flows.
- The Wallet Provider and Rust core now support deletion of attestations, as
  well as their corresponding private keys.
...
- Sections of both the wallet and issuer code have been updated to be compliant
  with OpenID4VCI 1.0:
    ...
    - The `IssuerMetadata` type has been updated to be fully compliant with
      version 1.0.
- Sections of both the wallet and verifier code have been updated to be
  compliant with OpenID4VP 1.0:
    ...
    - OpenID4VP response encryption metadata now uses
      `encrypted_response_enc_values_supported`, and the wallet selects a
      mutually supported JWE `enc` value for the response.

## Removed features

- Support for DigiD app2app is removed, as it is technically not possible to
  support it with the Authorization Code Flow (ACF) of OpenID4VCI 1.0.

## Bug fixes

- The `lst` field of a StatusList was not properly encoded and missing the last
  few bytes.
