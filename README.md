...
cargo install --locked --version 2.12.0 flutter_rust_bridge_codegen
...
| ALLOW_RELEASE_LOGS              | Dart define | Flutter, Cargo option_env           | Allows profile/release builds to emit Flutter and Rust device logs and forward those logs to Sentry Logs when Sentry is configured. Defaults to `false`; release builds must opt in explicitly, e.g. ont/demo. Debug builds still enable logs by build mode. |
