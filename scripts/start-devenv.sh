...
then
    ...
    then
        ...
        RUST_LOG=debug cargo run --package pid_issuer --no-default-features --features "admin_ui,test_api,postgres" --bin pid_issuer > "${TARGET_DIR}/pid_issuer.log" 2>&1 &
...
then
    ...
    then
        ...
        DATABASE_URL="postgres://${DB_USERNAME}:${DB_PASSWORD}@${DB_HOST}:${DB_PORT}/verification_server" cargo run --package verification_server_migrations --bin verification_server_migrations -- fresh
        popd
...
then
    ...
    then
        ...
        RUST_LOG=debug cargo run --package issuance_server --no-default-features --features "admin_ui,allow_insecure_url,test_api,postgres" --bin issuance_server > "${TARGET_DIR}/demo_issuer_issuance_server.log" 2>&1 &
...
then
    ...
    then
        ...
        RUST_LOG=debug cargo run --package wallet_provider --bin wallet_provider --features=admin_ui,android_emulator,test_api > "${TARGET_DIR}/wallet_provider.log" 2>&1 &
