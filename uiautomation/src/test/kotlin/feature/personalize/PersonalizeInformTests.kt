...
class PersonalizeInformTests : TestBase() {
    ...
    @DisplayName("$USE_CASE.1 The app first displays a screen to the user explaining that to personalize their wallet they must log in with the DigiD App that needs to be activated at level High.")
    fun verifyPersonalizeInformScreen() {
        assertTrue(personalizeInformScreen.visible(), "personalize inform screen is not visible")
    }

    @RetryingTest(value = MAX_RETRY_COUNT, name = "{displayName} - {index}")
    @DisplayName("$USE_CASE.2 The app offers guidance for when the user has no DigiD: the user is referred to the DigiD website.")
    ...
    @RetryingTest(value = MAX_RETRY_COUNT, name = "{displayName} - {index}")
    @DisplayName("$USE_CASE.3 The app offers a button for the user to Log in with DigiD.")
    fun verifyLoginDigidButton() {
        personalizeInformScreen.loginWithDigidButtonVisible()
    }

    @RetryingTest(value = MAX_RETRY_COUNT, name = "{displayName} - {index}")
    @DisplayName("$USE_CASE.4 When the user hits 'Log in with DigiD', DigiD authentication starts.")
