...
- and then create an APK by executing `CONFIG_ENV=ont UNIVERSAL_LINK_BASE=https://app.example.com/deeplink/ bundle exec fastlane android build build_mode:profile file_format:apk mock_relying_party_url:https://example.com/ universal_link_base:app.example.com`.
- and then create an IPA by executing `CONFIG_ENV="ont" UL_HOSTNAME=app.example.com UNIVERSAL_LINK_BASE="https://app.example.com/deeplink/" bundle exec fastlane ios build app_store:false build_mode:profile mock_relying_party_url:https://example.com/ universal_link_base:app.example.com`.
