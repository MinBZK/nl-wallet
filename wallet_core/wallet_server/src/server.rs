...
use crate::{
    issuer::create_issuance_router, settings::Settings, store::SessionStores, verifier::create_verifier_routers,
};
...
pub async fn serve<A>(settings: &Settings, sessions: SessionStores, attr_service: A) -> Result<()>
where
    A: AttributeService,
{
    ...
    let (wallet_router, requester_router) = create_verifier_routers(settings.clone(), sessions.disclosure)?;
    let issuance_router = create_issuance_router(settings.clone(), sessions.issuance, attr_service).await?;
