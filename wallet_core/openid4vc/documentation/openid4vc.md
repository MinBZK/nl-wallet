...
- This implementation uses the DPoP (Demonstrating Proof of Possession, [RFC 9449](https://datatracker.ietf.org/doc/html/rfc9449)) mechanism: when the holder requests the attestations by sending its signatures over the nonce, it provides not only the access token by way of authentication, but it also signs the access token with an ephemeral ECDSA private key that it also previously used when requesting the nonce, to ensure that only the wallet can use the access token to retrieve the attestations.
