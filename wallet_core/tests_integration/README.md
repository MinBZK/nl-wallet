...
- `VERIFICATION_SERVER_URL`: The public URL of the Verification Server where the status for the session can be retrieved.
...
To run the test locally, the following servers need to be started:
    ...
    ./scripts/start-devenv.sh postgres wallet_provider static_server update_policy_server verification_server pid_issuer digid_connector brp
