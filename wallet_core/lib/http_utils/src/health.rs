...
mod tests {
    ...
    use super::HealthStatus::DOWN;
    use super::HealthStatus::UP;
    use super::*;
    ...
    fn test_create_health_router_checks_duplicates() {
        let result = catch_unwind(|| {
            let a1 = Box::new(TestChecker::ok("a", UP)) as Box<dyn HealthChecker>;
            let a2 = Box::new(TestChecker::ok("a", UP)) as Box<dyn HealthChecker>;
    ...
    #[case::one_up(vec![TestChecker::ok("a", UP)], StatusCode::OK, json!(
        {"status": "UP", "checks": [{"name": "a", "status": "UP"}]}
    ))]
    #[case::one_down(vec![TestChecker::ok("b", DOWN)], StatusCode::SERVICE_UNAVAILABLE, json!(
        {"status": "DOWN", "checks": [{"name": "b", "status": "DOWN"}]}
    ))]
    #[case::one_up_one_down(vec![TestChecker::ok("a", UP), TestChecker::ok("b", DOWN)], StatusCode::SERVICE_UNAVAILABLE, json!(
        {"status": "DOWN", "checks": [{"name": "a", "status": "UP"}, {"name": "b", "status": "DOWN"}]}
    ))]
    #[case::one_up_one_err(vec![TestChecker::ok("a", UP), TestChecker::err("c", "reason")], StatusCode::SERVICE_UNAVAILABLE, json!(
        {"status": "DOWN", "checks": [{"name": "a", "status": "UP"}, {"name": "c", "status": "DOWN", "data": {"error": "reason"}}]}
    ))]
