...
///   as `04 || x || y` where `||` is bitwise concatenation. Note that this encodes just the public key, and it does
///   not include any information on the particular curve that is used, of which the public key is an element. In case
///   of JWTs this is okay, because in that case that information is transmitted elsewhere: in the `alg` field of the
///   JWT header, which in our case is `ES256` - meaning the `secp256r` curve. This encoding is what
