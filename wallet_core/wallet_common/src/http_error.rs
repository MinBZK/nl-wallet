...
///
/// * `r#type` \
/// A short string category for the error. This is the only mandatory field. Its type is generic and
/// encoding to and from a string is enforced by the [`Display`] and [`FromStr`] trait bounds.
/// * `title` \
/// An optional a short summary of the error type.
/// * `status` \
/// An optional HTTP status code for the error response. In the `IntoResponse` implementation this will be
/// used as the actual status code. If this field is `None`, as 500 status code will be used.
/// * `detail` \
/// Optional detailed and specific information about the error.
/// * `instance` \
/// An optional unique URI referencing the error.
/// * `extra` \
/// This may contain extra key-value pairs of the JSON object, as allowed by the RFC. As this uses both the
/// [`Map`] and [`Value`] types from the `serde_json` crate, (de)serialization of [`HttpJsonErrorBody`] instances is
/// restricted to JSON.
///
